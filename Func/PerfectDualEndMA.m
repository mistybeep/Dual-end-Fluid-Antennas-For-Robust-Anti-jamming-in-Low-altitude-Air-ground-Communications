function rate = PerfectDualEndMA(sim, data, norm_A, lambda, M, N_G, N_L, N_J, N_A, K, P_dBm, P_J_dBm, sigma2_dBm, gamma, err_range, alpha, rho_0_dB, Q, R, w_L, v_A, EpsKappa)
    ...
    for l = 1:L_max
        ...
        [v_A, mu, nu, ~, r_A] = find_v_A_Per(H_GA, H_LA, W_G, w_L, v_A, P_J, Sigma_LA, G_LA, t_L, Sigma_GA, G_GA, t_G, Sigma_JA, G_JA, mu, nu, Q, R, data, err_range, sigma2, t_J, r_A, DeltaPRM, EpsilonPRM, lambda, norm_A);
        [H_GA, F_GA, G_GA] = generate_channel(t_G, r_A, lambda, data.theta_GA_t, data.phi_GA_t, data.theta_GA_r, data.phi_GA_r, Sigma_GA, false);
        [H_LA, F_LA, G_LA] = generate_channel(t_L, r_A, lambda, data.theta_LA_t, data.phi_LA_t, data.theta_LA_r, data.phi_LA_r, Sigma_LA, false);
